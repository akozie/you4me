...
class UpcomingDatesRecyclerAdapter(private val activity: Activity, private val listener: CalendarResultListener, private val dates: UpcomingDates, private val context: Context) : RecyclerView.Adapter<UpcomingDatesRecyclerAdapter.MyViewHolder>() {
    ...
    private fun addEventToCalendar(title: String, location: String, dateTime: LocalDateTime) {
...
//        context.startActivity(intent)
    ...
    fun startActivityForCalendarEvent(intent: Intent) {
        // Ensure the context is an activity
//        val activity = (listener as? Activity) ?: return
        activity.startActivityForResult(intent, ADD_EVENT_REQUEST_CODE)
    }
